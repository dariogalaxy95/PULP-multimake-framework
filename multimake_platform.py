...
for conf_num in range(len(platform)):

    source_cmd = 'source ' + toolchain + ' && source ' + platform_path + platform[conf_num]  # configuration of toolchain and virtual platform
    make_string = source_cmd + ' && make clean all ' + compilation_parameters
    if (compilation_parameters != ""):
        print('Platform: ' + platform[conf_num].replace('.sh','') + ', Configuration: ' + compilation_parameters + '\n')
    else:
        print('Platform: ' + platform[conf_num].replace('.sh','') + ', Configuration: ' + 'no parameters' + '\n')
...
for i in range(len(resultlist)):
    conf_name.append(platform[i].replace('.sh', ''))
...
for i in range(len(ovh)):
    ...
    ax_ovh[i].pie(percentage[i], labels=labels[i], shadow=True, autopct='%1.1f%%', labeldistance=1.03)
    ax_ovh[i].axis('equal')  # Equal aspect ratio ensures that pie is drawn as a circle.

    ax_ovh[i].set_title(platform[i].replace('.sh', ''))
