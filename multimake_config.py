...
if isinstance(json_data['makefile_path'], list):
    makefile_path = json_data['makefile_path']
else:
    makefile_path.append(json_data['makefile_path'])

if isinstance(json_data['compilation_parameters'], list):
    compilation_parameters = json_data['compilation_parameters']
else:
    compilation_parameters.append(json_data['compilation_parameters'])
...
for mk_num in range(len(makefile_path)):
    for conf_num in range(len(compilation_parameters)):
        ...
        process = subprocess.run(make_string, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, shell=True, cwd=makefile_path[mk_num], executable='/bin/bash')
        make_string = source_cmd + ' && make run -s'
        process = subprocess.run(make_string, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, shell=True, cwd=makefile_path[mk_num], encoding='UTF-8', executable='/bin/bash')

        print(process.stdout)

        # Get the number of cores

        num_cores = GetCores(process.stdout)
...
if (opname != ""):
    fig, (ax_cycles, ax_opcycles, ax_pwr) = plt.subplots(1, 3, figsize=(12,7))
else:
    fig, (ax_cycles, ax_pwr) = plt.subplots(1, 2, figsize=(12,7))
...
for i in range(len(resultlist)):
    conf_name.append('Conf ' + str(i+1))

    values.append(GetMean(resultlist[i]['num_cycles']))
    values_active.append(GetMean(resultlist[i]['num_active_cycles']))
    #rects_total.append(ax_cycles.bar(x - width/2 + i*width + i*0.1, resultlist[i]['num_cycles'][0], width, color='orange'))
    #rects_active.append(ax_cycles.bar(x - width/2 + i*width + i*0.1, resultlist[i]['num_active_cycles'][0], width, color='red'))
...
#color_palette  = [(31,119,180), (255,127,14), (44,160,44), (214,39,40)]
...
for k in range(len(ovh)):
    ...
    for i in range(len(ovh[k])):
        if (perc[k][i] != 0):  # excluding zero values for overlapping reasons
            ...
            clr_tmp.append(color_palette[i])
    labels.append(lbl_tmp)
    percentage.append(perc_temp)
    colors.append(clr_tmp)
...
for i in range(len(ovh)):
    ...
    ax_ovh[i].pie(percentage[i], labels=labels[i], shadow=True, autopct='%1.1f%%', colors=colors[i])
    ax_ovh[i].axis('equal')  # Equal aspect ratio ensures that pie is drawn as a circle.

    ax_ovh[i].set_title('Conf ' + str(i+1))
